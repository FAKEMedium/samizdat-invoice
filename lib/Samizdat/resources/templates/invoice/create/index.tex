...
% my $ccy = lc($customer->{currency} // 'sek');
% my $is_swedish_customer = ($customer->{billingcountry} && $customer->{billingcountry} eq 'SE');
% my $currency_symbols = { sek => ($is_swedish_customer ? 'kr' : 'SEK'), eur => '\texteuro'};
% my $currency = $currency_symbols->{$ccy} // uc($ccy);
% my $countries = countrylist;
% my $vat = $formdata->{vat};
% my $is_reverse_charge = ($customer->{billingcountry} && $customer->{billingcountry} ne 'SE' && ($customer->{company} || $customer->{vatno}) && $vat == 0) ? 1 : 0;
...
% }
% if ('SE' eq $customer->{billingcountry}) {
...
\footnotesize {<%== __('Approved for F-tax') %>}\newline
% }
\vfill
% if ($customer->{billinglang} && $customer->{billinglang} =~ /^(sv)/i) {
%   $invoice->{totalcost} =~ tr/\./,/;
%   $invoice->{vatcost} =~ tr/\./,/;
%   $invoice->{diff} =~ tr/\./,/;
%   $invoice->{net_amount} =~ tr/\./,/;
%   $vat =~ tr/\./,/;
% }
% if ('eur' eq $ccy && (!$customer->{billinglang} || $customer->{billinglang} !~ /^(sv)/i)) {
{\huge \textbf {\
%   if (exists($invoice->{kreditfakturaavser}) && $invoice->{kreditfakturaavser}) {
<%== __x('Credited amount incl. VAT: {totalcost} {currency}', totalcost => $invoice->{totalcost},	currency => $currency) %>\
%   } else {
<%== __x('Amount to pay incl. VAT: {totalcost} {currency}', totalcost => $invoice->{totalcost},	currency => $currency) %>\
%   }
}}
\vfill
\footnotesize {\textbf {<%== __('Net amount') %>:} <%== $invoice->{net_amount} %> <%=$currency %>}\newline
\footnotesize {\textbf {<%== __x('VAT {rate}\%', rate => $vat) %><%= $is_reverse_charge ? ' (' . __('reverse charge') . ')' : '' %>:} <%== $invoice->{vatcost} %> <%=$currency %>}\newline
% } else {
{\huge \textbf {\
%   if (exists($invoice->{kreditfakturaavser}) && $invoice->{kreditfakturaavser}) {
<%== __x('Credited amount incl. VAT: {totalcost} {currency}', totalcost => $invoice->{totalcost},	currency => $currency) %>\
...
\footnotesize {\textbf {<%== __x('VAT {rate}\%', rate => $vat) %><%= $is_reverse_charge ? ' (' . __('reverse charge') . ')' : '' %>:} <%== $invoice->{vatcost} %> <%=$currency %>}\newline
%   if ($invoice->{diff}) {
\footnotesize {\textbf {<%== __('Rounding') %>:} <%== $invoice->{diff} %> <%=$currency %>}\newline
%   }
% }
% if (!(exists($invoice->{kreditfakturaavser}) && $invoice->{kreditfakturaavser})) {
...
%   # LaTeX escape the invoice item text
%   my $itemtext = tex_escape($invoiceitem->{invoiceitemtext} // '');
%   $itemtext =~ s/--/-\\mbox{}-/g;  # Handle double dashes for IDN domains like xn--example.se
%
...
%   }
\small {<%== __x('{cost}', cost => $cost, currency => $currency) %>} & \
\small {<%== __x('{cost}', cost => $net, currency => $currency) %>} & \
\small {<%== __x('{cost}', cost => $vatcost, currency => $currency) %>} & \
\small {<%== __x('{cost}', cost => $gross, currency => $currency) %>}\\\
